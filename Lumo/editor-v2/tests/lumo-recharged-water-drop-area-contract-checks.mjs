...

{
  ...
  const assertOrdered = (source, earlier, later, message) => {
    const earlierIndex = source.indexOf(earlier);
    const laterIndex = source.indexOf(later);
    assert.notEqual(earlierIndex, -1, `${message}: missing earlier marker ${earlier}`);
    assert.notEqual(laterIndex, -1, `${message}: missing later marker ${later}`);
    assert.ok(earlierIndex < laterIndex, message);
  };
  const lumoFrameLaneSource = lumoSource.slice(lumoSource.indexOf("// Draw support tiles"), lumoSource.indexOf("const liquidEntityTypes = new Set"));
  assertOrdered(lumoFrameLaneSource, "drawRechargedStoneAreas(ctx, mapper, stoneAreas", "drawRechargedWaterDropAreas(ctx, mapper, waterDropAreas", "Lumo.html renders Water Drop Areas after Stone Areas");
  assertOrdered(lumoFrameLaneSource, "for (const supportTile of supportTiles)", "drawRechargedWaterDropAreas(ctx, mapper, waterDropAreas", "Lumo.html renders Water Drop rings above impacted support tile surfaces");
  assertOrdered(lumoFrameLaneSource, "renderReactiveGrass(", "drawRechargedWaterDropAreas(ctx, mapper, waterDropAreas", "Lumo.html renders Water Drop Areas after reactive grass");
  assertOrdered(lumoFrameLaneSource, "renderReactiveBloomPlants(", "drawRechargedWaterDropAreas(ctx, mapper, waterDropAreas", "Lumo.html renders Water Drop Areas after reactive bloom");
  assertOrdered(lumoFrameLaneSource, "renderReactiveCrystals(", "drawRechargedWaterDropAreas(ctx, mapper, waterDropAreas", "Lumo.html renders Water Drop Areas after reactive crystals");
  assertOrdered(lumoFrameLaneSource, "drawRechargedDecorVisual(ctx, mapper, state, decor", "drawRechargedWaterDropAreas(ctx, mapper, waterDropAreas", "Lumo.html renders Water Drop Areas after static/foreground decor");
  assertOrdered(lumoFrameLaneSource, "drawRechargedMirrorSurfaceAreas(ctx, mapper, state, mirrorSurfaceAreas", "drawRechargedWaterDropAreas(ctx, mapper, waterDropAreas", "Lumo.html renders Water Drop Areas after mirror/world visuals while preserving reflection inputs");
  assertOrdered(lumoSource.slice(lumoSource.indexOf("// Foreground atmospheric FX: Water Drop Area visuals")), "drawRechargedWaterDropAreas(ctx, mapper, waterDropAreas", "const liquidEntityTypes = new Set", "Lumo.html renders Water Drop foreground FX before entity/player/HUD lanes");
  assert.equal((lumoFrameLaneSource.match(/drawRechargedWaterDropAreas\(ctx, mapper, waterDropAreas/g) || []).length, 1, "Lumo.html draws Water Drop Areas exactly once per frame");
  const bridgeFrameLaneSource = bridgeSource.slice(bridgeSource.indexOf("export function drawRuntimeBridgeView"));
  assertOrdered(bridgeFrameLaneSource, "for (const tile of tiles)", "renderRuntimeWaterDropAreas", "runtime bridge renders Water Drop rings above impacted tile surfaces");
  assertOrdered(bridgeFrameLaneSource, "ctx.fillRect(worldToScreen(cameraState, x - 4)", "renderRuntimeWaterDropAreas", "runtime bridge renders Water Drop Areas after static decor markers");
  assertOrdered(bridgeFrameLaneSource, "renderRuntimeWaterDropAreas", "const entities = Array.isArray(viewModel?.entities)", "runtime bridge keeps Water Drop Areas below entity/debug/HUD lanes");
  const editorFrameLaneSource = fs.readFileSync(rendererPath, "utf8").slice(fs.readFileSync(rendererPath, "utf8").indexOf("export function renderEditorFrame"));
  assertOrdered(editorFrameLaneSource, "renderStoneAreas(worldCtx", "renderWaterDropAreas(worldCtx", "Editor V2 preview renders Water Drop Areas after Stone Areas");
  assertOrdered(editorFrameLaneSource, "renderReactiveGrassPatches(worldCtx", "renderWaterDropAreas(worldCtx", "Editor V2 preview renders Water Drop Areas after reactive grass");
  assertOrdered(editorFrameLaneSource, "renderReactiveBloomPatches(worldCtx", "renderWaterDropAreas(worldCtx", "Editor V2 preview renders Water Drop Areas after reactive bloom");
  assertOrdered(editorFrameLaneSource, "renderReactiveCrystalPatches(worldCtx", "renderWaterDropAreas(worldCtx", "Editor V2 preview renders Water Drop Areas after reactive crystals");
  assertOrdered(editorFrameLaneSource, "renderDecor(worldCtx", "renderWaterDropAreas(worldCtx", "Editor V2 preview renders Water Drop Areas after decor");
  assertOrdered(editorFrameLaneSource, "renderWaterDropAreas(worldCtx", "renderProximityOverlays(ctx", "Editor V2 preview keeps Water Drop Areas below editor overlays/UI");
  assert.equal((editorFrameLaneSource.match(/renderWaterDropAreas\(worldCtx/g) || []).length, 1, "Editor V2 preview draws Water Drop Areas exactly once per frame");

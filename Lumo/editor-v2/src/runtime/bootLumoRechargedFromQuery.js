...
export async function bootLumoRechargedFromQuery(options = {}) {
  ...
  try {
    ...
    return createBaseResult({
      ...
      worldWidth: Number.isFinite(payload.worldWidth) ? payload.worldWidth : null,
      worldHeight: Number.isFinite(payload.worldHeight) ? payload.worldHeight : null,
      ...
      layers: {
        mirrorSurfaceAreas: Array.isArray(payload?.layers?.mirrorSurfaceAreas) ? payload.layers.mirrorSurfaceAreas : [],
      },

...
function buildWorldSnapshot(snapshot) {
  ...
  const decorItems = Array.isArray(source.decorItems)
    ? source.decorItems
      ...
      // Keep authored order deterministic: order asc, then source insertion order.
  ...
  try {
    ...
    function getBootPayload() {
      ...
      return {
        ...
        reactiveGrassPatches: world.reactiveGrassPatches,
        reactiveBloomPatches: world.reactiveBloomPatches,
        reactiveCrystalPatches: world.reactiveCrystalPatches,
        playerStatus: player.locomotion,
        playerX: player.x,
        playerY: player.y,
        ...
        playerRenderAlpha: Number.isFinite(player.renderAlpha) ? player.renderAlpha : 1,

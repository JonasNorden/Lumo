...
const DEFAULT_PULSE_START_RADIUS_PX = 52;
...
const DEATH_FALL_MAX_DISTANCE_TILES_TIGHT = 0.5;
...
function isPlayerLikelyOnMovingPlatform(sourceEntities, playerState, tileSize) {
  const onPlatformId = typeof playerState?.onPlatformId === "string" ? playerState.onPlatformId : null;
  if (!onPlatformId || !Array.isArray(sourceEntities)) {
    return false;
  }
  const platform = sourceEntities.find((entity) => entity?.id === onPlatformId && isMovingPlatformEntityType(entity?.type));
  if (!platform) {
    return false;
  }
  const runtime = resolveMovingPlatformRuntimeState(platform, tileSize);
  const platformTop = Number.isFinite(platform?.y) ? platform.y : runtime.originY;
  const playerFeet = Number.isFinite(playerState?.position?.y) ? playerState.position.y + 1 : null;
  if (!Number.isFinite(platformTop) || !Number.isFinite(playerFeet)) {
    return false;
  }
  return Math.abs(playerFeet - platformTop) <= (MOVING_PLATFORM_LANDING_EPSILON_PX + 1);
}
...
function stepFlareEntityInteractions(sourceEntities, flares) {
  ...
  const entities = sourceEntities.map((entity) => {
    ...
    if (illuminated) {
      // Keep V1-compatible flare suppression contract: illumination forces suppressed state.
...
function stepFlares(worldPacket, playerState, intent, options = {}) {
  ...
  for (const flare of existingFlares) {
    ...
    const lightSnapshot = buildFlareLightSnapshot(steppedFlare);
    nextFlares.push({
      ...steppedFlare,
      alpha: Number.isFinite(lightSnapshot?.alpha) ? lightSnapshot.alpha : steppedFlare.alpha,
  ...
  if (throwAllowed) {
    const spawnedFlare = buildFlareProjectile(playerState, facingX, nextFlareId);
    const lightSnapshot = buildFlareLightSnapshot(spawnedFlare);
    nextFlares.push({
      ...spawnedFlare,
      alpha: Number.isFinite(lightSnapshot?.alpha) ? lightSnapshot.alpha : spawnedFlare.alpha,
...
function resolvePlayableBottomY(worldPacket) {
  const tileSize = worldPacket?.world?.tileSize;
  const worldHeight = worldPacket?.world?.height;
  ...
  if (Number.isFinite(tileBoundsMaxY)) {
    return resolveWorldYUnit(tileBoundsMaxY);
  }

  if (Number.isFinite(worldHeight) && worldHeight > 0) {
    return resolveWorldYUnit(worldHeight);
  }

  return null;
}

function resolveWorldBottomFallbackY(worldPacket, tileSize) {
  const playableBottomY = resolvePlayableBottomY(worldPacket);
  if (!Number.isFinite(playableBottomY) || !Number.isFinite(tileSize) || tileSize <= 0) {
    return null;
  }
  return playableBottomY - (tileSize * 2);
}

function resolveGameplayDeathPlaneY(worldPacket, sourceEntities = []) {
  ...
  const explicitFallHazardTopY = [
    ...
    worldPacket?.bounds?.fallHazardTopY,
  ].find((value) => Number.isFinite(value));
  if (Number.isFinite(explicitFallHazardTopY)) {
    return {
  ...
  const movingPlatformBottomY = Array.isArray(sourceEntities)
    ? sourceEntities.reduce((maxBottom, entity) => {
      if (!isMovingPlatformEntityType(entity?.type)) {
        return maxBottom;
      }
      const runtime = resolveMovingPlatformRuntimeState(entity, tileSize);
      const platformTop = Math.max(
        Number.isFinite(entity?.y) ? entity.y : Number.NEGATIVE_INFINITY,
        Number.isFinite(runtime?.originY) ? runtime.originY : Number.NEGATIVE_INFINITY,
        Number.isFinite(runtime?.endY) ? runtime.endY : Number.NEGATIVE_INFINITY,
      );
      if (!Number.isFinite(platformTop)) {
        return maxBottom;
      }
      const platformHeight = Number.isFinite(runtime?.heightPx) && runtime.heightPx > 0 ? runtime.heightPx : tileSize;
      return Math.max(maxBottom, platformTop + platformHeight - 1);
    }, Number.NEGATIVE_INFINITY)
    : Number.NEGATIVE_INFINITY;
  const contentBottomY = Math.max(supportBottomY, movingPlatformBottomY);
  if (Number.isFinite(contentBottomY)) {
    return {
  ...
  return {
    deathPlaneY: resolveWorldBottomFallbackY(worldPacket, tileSize),
    source: "world-bottom-minus-two-tiles",
  };
}
...
function resolveBottomBoundRespawnY(worldPacket, options = {}) {
  ...
  const marginTiles = Number.isFinite(options?.bounds?.fallRespawnMarginTiles)
    ...
    : 4;
  const playableBottomY = resolvePlayableBottomY(worldPacket);
  if (!Number.isFinite(tileSize) || tileSize <= 0 || !Number.isFinite(playableBottomY)) {
    return null;
  }
  ...
  if (livesAfterDeath <= 0) {
    return {
      player: applyPlayerLightParity(buildGameOverPlayerState({
        ...playerState,
        position: {
          x: Number.isFinite(spawnX) ? spawnX : triggerPosition?.x,
          y: Number.isFinite(spawnY) ? spawnY : triggerPosition?.y,
        },
        lives: 0,
      })),
      debug: {
...
function resolveHazardFallTracker(worldPacket, playerState, verticalStep, sourceEntities) {
  const tileSize = Number.isFinite(worldPacket?.world?.tileSize) && worldPacket.world.tileSize > 0
    ? worldPacket.world.tileSize
    : 24;
  ...
  const grounded = verticalStep?.grounded === true;
  const onPlatform = (
    typeof verticalStep?.onPlatformId === "string"
    || isPlayerLikelyOnMovingPlatform(sourceEntities, playerState, tileSize)
  );
  ...
  const armReason = outOfBounds
    ? (forcedOutOfBoundsArm
        ? "forced-below-death-plane-plus-threshold"
        : `below-${deathPlane?.source ?? "death-plane"}`)
    : (!Number.isFinite(currentY)
        ? "no-current-y"
        : (!Number.isFinite(deathPlaneY)
            ? "no-death-plane"
            : (grounded ? "grounded" : (onPlatform ? "on-platform" : (descending ? "above-death-plane" : "not-descending")))));
  const hazardType = liquidType
    ? `liquid:${liquidType}`
    : (outOfBounds ? "out-of-bounds" : null);
  const sourceTracker = playerState?._fallTracker && typeof playerState._fallTracker === "object"
    ? playerState._fallTracker
    : null;
  ...
  const farBeyondDeathPlane = (
    forcedOutOfBoundsArm
    && Number.isFinite(currentY)
    && Number.isFinite(effectiveDeathPlaneY)
    && Number.isFinite(threshold)
    && threshold > 0
    && currentY > (effectiveDeathPlaneY + (threshold * 3))
  );
  const initialFallStartY = farBeyondDeathPlane && Number.isFinite(effectiveDeathPlaneY)
...
function maybeResolveBottomRespawn(worldPacket, playerState, verticalStep, hazardFall, options = {}) {
  const currentY = Number.isFinite(verticalStep?.position?.y) ? verticalStep.position.y : null;
  const shouldRespawn = hazardFall?.outOfBounds === true && hazardFall?.triggered === true;
  const respawnY = Number.isFinite(hazardFall?.deathPlaneY)
    ? hazardFall.deathPlaneY
    : resolveBottomBoundRespawnY(worldPacket, options);
  ...
  return buildDeathRespawnState(worldPacket, playerState, verticalStep.position, {
    ...
    triggerY: currentY,
    respawnY,
    warning: "Player fell below playable world bounds; reached max death-fall distance and respawn countdown started at authored spawn.",
  });
}

function maybeResolveLiquidDeath(worldPacket, playerState, verticalStep, sourceEntities, hazardFall, options = {}) {
  // Lock lethal-liquid ownership once respawn handoff starts so a second sink cannot retrigger mid-transition.
  if (isRespawnHandoffLocked(playerState)) {
    return null;
  }
  const liquidType = typeof hazardFall?.liquidType === "string"
    ? hazardFall.liquidType
    : null;
  if (!liquidType) {
    return null;
  }
  ...
  const reachedMaxFallDistance = hazardFall?.triggered === true;

  if (reachedMaxFallDistance) {
    return buildDeathRespawnState(worldPacket, {
      ...
      triggerY: Number.isFinite(hazardFall?.fallStartY) ? hazardFall.fallStartY : baseY,
      warning: `Player sank in ${liquidType}; reached max death-fall distance and respawn countdown started.`,
    });
  }
...
export function stepRuntimePlayerSimulation(worldPacket, playerState, options = {}) {
  ...
  if (respawnCountdown.active) {
    const pendingIntent = buildRuntimePlayerIntent(options?.input ?? options?.intent ?? options);
    const pendingPulseStep = stepPulse(playerState, pendingIntent, options);
    const pendingEntitiesSource = normalizeRuntimeEntities(playerState?.entities, worldPacket, playerState);
    const pendingEntityStep = stepPulseEntityInteractions(
      worldPacket,
      playerState,
      pendingPulseStep.pulse,
      pendingEntitiesSource,
      options,
    );
    const pending = buildRespawnPendingPlayerState(playerState, respawnCountdown, options);
    const pendingMoveX = Number.isFinite(pendingIntent?.moveX) ? pendingIntent.moveX : 0;
    const pendingBaseX = Number.isFinite(playerState?.position?.x) ? playerState.position.x : 0;
    const pendingBaseY = Number.isFinite(playerState?.position?.y) ? playerState.position.y : 0;
    const pendingPosition = {
      x: pendingBaseX + pendingMoveX * 2,
      y: pendingIntent?.jump === true ? pendingBaseY - 1 : pendingBaseY,
    };
    if (pending.waiting) {
      ...
      return {
        ...
        player: applyPlayerLightParity({
          ...
          pulse: pendingPulseStep.pulse,
          pulseHeldLastTick: pendingPulseStep.pulseHeldLastTick,
          entities: pendingEntityStep.entities,
          lives,
          liquidDeath: null,
          renderAlpha: 1,
  ...
  const finalEntities = Array.isArray(hoverVoidStep?.entities)
    ? hoverVoidStep.entities.map((entity) => {
      if (resolvePulseTargetType(entity?.type) === "dark_creature" && entity?.illuminated === true) {
        return {
          ...entity,
          state: "suppressed",
          consumesFlare: true,

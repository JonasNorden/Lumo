...
function ensureClusterCache(c){const k=clusterKey(c);const v=crystalCacheByCluster.get(k);if(v)return {key:k,localClusters:v};const localClusterGeometry=buildCrystalClusters(c);const localClusters=[localClusterGeometry];crystalCacheByCluster.set(k,localClusters);return {key:k,localClusters};}

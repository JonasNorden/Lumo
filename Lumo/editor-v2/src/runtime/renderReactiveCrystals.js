...
function ensureClusterCache(c){const k=clusterKey(c);const cached=crystalCacheByCluster.get(k);if(cached!==undefined){const localClusters=Array.isArray(cached)?cached:[cached];if(!Array.isArray(cached)) crystalCacheByCluster.set(k,localClusters);return {key:k,localClusters};}const localClusterGeometry=buildCrystalClusters(c);const localClusters=[localClusterGeometry];crystalCacheByCluster.set(k,localClusters);return {key:k,localClusters};}

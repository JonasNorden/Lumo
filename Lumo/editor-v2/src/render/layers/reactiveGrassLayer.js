...
export function renderReactiveGrassPatches(ctx, doc, viewport) {
  ...
  for (let index = 0; index < patches.length; index += 1) {
    ...
    const topLeft = worldToCanvas(viewport, leftWorldX, topWorldY);
    const bottomRight = worldToCanvas(viewport, leftWorldX + patch.width, patch.y);
    ...
    ctx.strokeStyle = withAlpha(patch.topColor, 0.76, DEFAULT_PATCH_COLORS.top);

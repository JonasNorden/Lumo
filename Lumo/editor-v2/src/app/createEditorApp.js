...
import {
  ...
  createSmokeAreaEditEntry,
  createWaterDropAreaEditEntry,
...
import { findSmokeAreaAtCanvasPoint } from "../render/layers/smokeAreaLayer.js";
import { findWaterDropAreaAtCanvasPoint } from "../render/layers/waterDropAreaLayer.js";
...
function renderCellHud(cellHud, state) {
  ...
  const activeTargetLabel = activeLayer === PANEL_LAYERS.BACKGROUND ? "Background" : activeLayer === PANEL_LAYERS.DECOR ? "Decor" : activeLayer === PANEL_LAYERS.ENTITIES ? "Entities" : activeLayer === PANEL_LAYERS.REACTIVE_DECOR ? "Reactive Decor" : activeLayer === PANEL_LAYERS.SOUND ? "Sound" : activeLayer === PANEL_LAYERS.WORLD_AREAS ? "World Areas" : "Tiles";
      ...
      : activeLayer === PANEL_LAYERS.ENTITIES
        ...
        : activeLayer === PANEL_LAYERS.SOUND
          ...
          : activeLayer === PANEL_LAYERS.WORLD_AREAS
            ? (state.interaction.selectedMirrorSurfaceAreaId ? 1 : 0)
          : 0;
...
function getActiveLayer(interaction) {
  ...
  if (interaction.activeLayer === PANEL_LAYERS.WORLD_AREAS) return PANEL_LAYERS.WORLD_AREAS;
...
function getSelectionMode(interaction) {
  ...
  if (interaction.canvasSelectionMode === "worldAreas") return "worldAreas";
  return "entity";
}
...
export function createEditorApp({
  ...
  const clearReactiveGrassPatchSelection = (interaction) => {
    ...
    interaction.selectedMirrorSurfaceAreaIndex = null;
    interaction.selectedMirrorSurfaceAreaId = null;
    interaction.selectedStoneAreaIndex = null;
    ...
    interaction.selectedSmokeAreaIndex = null;
    interaction.selectedSmokeAreaId = null;
    interaction.selectedWaterDropAreaIndex = null;
    interaction.selectedWaterDropAreaId = null;
  };
  ...
  const setMirrorSurfaceAreaSelection = (draft, areaIndex = null) => {
    const areas = Array.isArray(draft.document.active?.mirrorSurfaceAreas) ? draft.document.active.mirrorSurfaceAreas : [];
    ...
    draft.interaction.selectedMirrorSurfaceAreaIndex = nextArea ? areaIndex : null;
    draft.interaction.selectedMirrorSurfaceAreaId = typeof nextArea?.id === "string" && nextArea.id.trim() ? nextArea.id.trim() : null;
  ...
  const setStoneAreaSelection = (draft, areaIndex = null) => {
    const areas = Array.isArray(draft.document.active?.stoneAreas) ? draft.document.active.stoneAreas : [];
    const nextArea = Number.isInteger(areaIndex) && areaIndex >= 0 && areaIndex < areas.length ? areas[areaIndex] : null;
    clearReactiveGrassPatchSelection(draft.interaction);
    ...
    clearDecorSelection(draft.interaction);
    clearSoundSelection(draft.interaction);
    draft.interaction.selectedStoneAreaIndex = nextArea ? areaIndex : null;
    draft.interaction.selectedStoneAreaId = typeof nextArea?.id === "string" && nextArea.id.trim() ? nextArea.id.trim() : null;
    draft.interaction.selectedCell = nextArea ? { x: Math.floor(Number(nextArea.x || 0) / Math.max(1, Number(draft.document.active?.dimensions?.tileSize) || 24)), y: Math.floor(Number(nextArea.y || 0) / Math.max(1, Number(draft.document.active?.dimensions?.tileSize) || 24)) } : null;
  };
  ...
  const setDustAreaSelection = (draft, areaIndex = null) => {
    const areas = Array.isArray(draft.document.active?.dustAreas) ? draft.document.active.dustAreas : [];
    const nextArea = Number.isInteger(areaIndex) && areaIndex >= 0 && areaIndex < areas.length ? areas[areaIndex] : null;
    clearReactiveGrassPatchSelection(draft.interaction);
    clearMirrorSurfaceAreaSelection(draft.interaction);
    clearStoneAreaSelection(draft.interaction);
  ...
  const setSmokeAreaSelection = (draft, areaIndex = null) => {
    const areas = Array.isArray(draft.document.active?.smokeAreas) ? draft.document.active.smokeAreas : [];
    const nextArea = Number.isInteger(areaIndex) && areaIndex >= 0 && areaIndex < areas.length ? areas[areaIndex] : null;
    clearReactiveGrassPatchSelection(draft.interaction);
    clearMirrorSurfaceAreaSelection(draft.interaction);
    clearStoneAreaSelection(draft.interaction);
    ...
    clearDecorSelection(draft.interaction);
    clearSoundSelection(draft.interaction);
    draft.interaction.selectedSmokeAreaIndex = nextArea ? areaIndex : null;
    draft.interaction.selectedSmokeAreaId = typeof nextArea?.id === "string" && nextArea.id.trim() ? nextArea.id.trim() : null;
    draft.interaction.selectedCell = nextArea ? { x: Math.floor(Number(nextArea.x || 0) / Math.max(1, Number(draft.document.active?.dimensions?.tileSize) || 24)), y: Math.floor(Number(nextArea.y || 0) / Math.max(1, Number(draft.document.active?.dimensions?.tileSize) || 24)) } : null;
  };

  const setWaterDropAreaSelection = (draft, areaIndex = null) => {
    const areas = Array.isArray(draft.document.active?.waterDropAreas) ? draft.document.active.waterDropAreas : [];
    const nextArea = Number.isInteger(areaIndex) && areaIndex >= 0 && areaIndex < areas.length ? areas[areaIndex] : null;
    clearReactiveGrassPatchSelection(draft.interaction);
    clearMirrorSurfaceAreaSelection(draft.interaction);
    clearStoneAreaSelection(draft.interaction);
    ...
    clearDecorSelection(draft.interaction);
    clearSoundSelection(draft.interaction);
    draft.interaction.selectedWaterDropAreaIndex = nextArea ? areaIndex : null;
    draft.interaction.selectedWaterDropAreaId = typeof nextArea?.id === "string" && nextArea.id.trim() ? nextArea.id.trim() : null;
    draft.interaction.selectedCell = nextArea ? { x: Math.floor(Number(nextArea.x || 0) / Math.max(1, Number(draft.document.active?.dimensions?.tileSize) || 24)), y: Math.floor(Number(nextArea.y || 0) / Math.max(1, Number(draft.document.active?.dimensions?.tileSize) || 24)) } : null;
  };
  ...
  const openLayerSection = (draft, layer) => {
    ...
    if (layer === PANEL_LAYERS.WORLD_AREAS) draft.ui.panelSections.worldAreas = true;
  };
  ...
  const setActiveLayer = (draft, layer) => {
    draft.interaction.activeLayer = layer === PANEL_LAYERS.BACKGROUND
      ...
      : layer === PANEL_LAYERS.ENTITIES
        ...
        : layer === PANEL_LAYERS.WORLD_AREAS
          ? PANEL_LAYERS.WORLD_AREAS
      ...
      : activeLayer === PANEL_LAYERS.ENTITIES
        ...
        : activeLayer === PANEL_LAYERS.SOUND
          ...
          : activeLayer === PANEL_LAYERS.WORLD_AREAS
            ? (state.interaction.selectedMirrorSurfaceAreaId ? 1 : 0)
          : 0;
    const activeSelectionLabel = activeLayer === PANEL_LAYERS.BACKGROUND ? "Background" : activeLayer === PANEL_LAYERS.DECOR ? "Decor" : activeLayer === PANEL_LAYERS.ENTITIES ? "Entities" : activeLayer === PANEL_LAYERS.REACTIVE_DECOR ? "Reactive Decor" : activeLayer === PANEL_LAYERS.SOUND ? "Sound" : activeLayer === PANEL_LAYERS.WORLD_AREAS ? "World Areas" : "Tiles";
  ...
  const applyCanvasTarget = (draft, mode) => {
    const nextMode = mode === "decor" ? "decor" : mode === "sound" ? "sound" : mode === "reactiveDecor" ? "reactiveDecor" : mode === "worldAreas" ? "worldAreas" : "entity";
  ...
  const updateWorldArea = (index, field, value) => {
    void index;
    void value;
    store.setState((draft) => {
      const requestedType = field === "arm-water-drop-area" ? "water_drop_area" : field === "arm-smoke-area" ? "smoke_area" : field === "arm-glow-area" ? "glow_area" : field === "arm-dust-area" ? "dust_area" : field === "arm-stone-area" ? "stone_area" : field === "arm-mirror-surface" ? "mirror_surface" : null;
      ...
      draft.interaction.activeSoundPresetId = null;
      draft.interaction.activeTool = EDITOR_TOOLS.INSPECT;
      draft.interaction.mirrorSurfaceAreaPlacementDrag = null;
      draft.interaction.mirrorSurfaceAreaDrag = null;
      draft.interaction.stoneAreaPlacementDrag = null;
      ...
      draft.interaction.smokeAreaPlacementDrag = null;
      draft.interaction.waterDropAreaPlacementDrag = null;
      draft.interaction.glowAreaDrag = null;
      draft.interaction.smokeAreaDrag = null;
  ...
  const deleteSelectedMirrorSurfaceArea = (draft) => {
    const areas = Array.isArray(draft.document.active?.mirrorSurfaceAreas) ? draft.document.active.mirrorSurfaceAreas : [];
    const selectedId = typeof draft.interaction.selectedMirrorSurfaceAreaId === "string" && draft.interaction.selectedMirrorSurfaceAreaId.trim() ? draft.interaction.selectedMirrorSurfaceAreaId.trim() : null;
    const selectedIndex = selectedId ? areas.findIndex((area) => area?.id === selectedId) : draft.interaction.selectedMirrorSurfaceAreaIndex;
    ...
    pushHistoryEntry(draft.history, createMirrorSurfaceAreaEditEntry("delete", {
  ...
  const deleteSelectedStoneArea = (draft) => {
    const areas = Array.isArray(draft.document.active?.stoneAreas) ? draft.document.active.stoneAreas : [];
    const selectedId = typeof draft.interaction.selectedStoneAreaId === "string" && draft.interaction.selectedStoneAreaId.trim() ? draft.interaction.selectedStoneAreaId.trim() : null;
    const selectedIndex = selectedId ? areas.findIndex((area) => area?.id === selectedId) : draft.interaction.selectedStoneAreaIndex;
    if (!Number.isInteger(selectedIndex) || selectedIndex < 0 || selectedIndex >= areas.length) return false;
    const previousArea = { ...areas[selectedIndex] };
    areas.splice(selectedIndex, 1);
    pushHistoryEntry(draft.history, createStoneAreaEditEntry("delete", {
      objectId: typeof previousArea.id === "string" ? previousArea.id : null,
      index: selectedIndex,
      previousSnapshot: previousArea,
    }));
  ...
  const deleteSelectedDustArea = (draft) => {
    const areas = Array.isArray(draft.document.active?.dustAreas) ? draft.document.active.dustAreas : [];
    const selectedId = typeof draft.interaction.selectedDustAreaId === "string" && draft.interaction.selectedDustAreaId.trim() ? draft.interaction.selectedDustAreaId.trim() : null;
    const selectedIndex = selectedId ? areas.findIndex((area) => area?.id === selectedId) : draft.interaction.selectedDustAreaIndex;
    if (!Number.isInteger(selectedIndex) || selectedIndex < 0 || selectedIndex >= areas.length) return false;
    const previousArea = { ...areas[selectedIndex] };
    areas.splice(selectedIndex, 1);
    pushHistoryEntry(draft.history, createDustAreaEditEntry("delete", {
      objectId: typeof previousArea.id === "string" ? previousArea.id : null,
      index: selectedIndex,
      previousSnapshot: previousArea,
    }));
  ...
  const updateMirrorSurfaceArea = (field, value, options = {}) => {
    ...
    store.setState((draft) => {
      ...
      const selectedAreaId = typeof options.areaId === "string" && options.areaId.trim()
        ? options.areaId.trim()
        : typeof draft.interaction.selectedMirrorSurfaceAreaId === "string" && draft.interaction.selectedMirrorSurfaceAreaId.trim()
          ? draft.interaction.selectedMirrorSurfaceAreaId.trim()
          : null;
      const selectedAreaIndex = Number.isInteger(draft.interaction.selectedMirrorSurfaceAreaIndex) ? draft.interaction.selectedMirrorSurfaceAreaIndex : -1;
      ...
      const nextArea = updateMirrorSurfaceAreaField(previousArea, field, value);
      if (JSON.stringify(previousArea) === JSON.stringify(nextArea)) return;
      areas[resolvedIndex] = nextArea;
      pushHistoryEntry(draft.history, createMirrorSurfaceAreaEditEntry("update", {
        objectId: typeof nextArea.id === "string" ? nextArea.id : null,
        index: resolvedIndex,
        previousSnapshot: previousArea,
        nextSnapshot: nextArea,
      }));
      setMirrorSurfaceAreaSelection(draft, resolvedIndex);
    });
  };
  ...
  const updateStoneArea = (field, value, options = {}) => {
    ...
    store.setState((draft) => {
      ...
      const selectedAreaId = typeof options.areaId === "string" && options.areaId.trim()
        ? options.areaId.trim()
        : typeof draft.interaction.selectedStoneAreaId === "string" && draft.interaction.selectedStoneAreaId.trim()
          ? draft.interaction.selectedStoneAreaId.trim()
          : null;
      const selectedAreaIndex = Number.isInteger(draft.interaction.selectedStoneAreaIndex) ? draft.interaction.selectedStoneAreaIndex : -1;
      const resolvedIndex = selectedAreaId ? areas.findIndex((area) => area?.id === selectedAreaId) : selectedAreaIndex;
      if (!Number.isInteger(resolvedIndex) || resolvedIndex < 0 || resolvedIndex >= areas.length) return;
      const previousArea = { ...areas[resolvedIndex] };
      const nextArea = updateStoneAreaField(previousArea, field, value);
      if (JSON.stringify(previousArea) === JSON.stringify(nextArea)) return;
      areas[resolvedIndex] = nextArea;
      pushHistoryEntry(draft.history, createStoneAreaEditEntry("update", {
        objectId: typeof nextArea.id === "string" ? nextArea.id : null,
        index: resolvedIndex,
        previousSnapshot: previousArea,
        nextSnapshot: nextArea,
      }));
      setStoneAreaSelection(draft, resolvedIndex);
    });
  };


  const updateDustArea = (field, value, options = {}) => {
    const supportedFields = new Set(["x", "y", "width", "height", "density", "sizeVariation", "driftStrength", "enabled", "visible"]);
    if (!supportedFields.has(field)) return;
    store.setState((draft) => {
      const areas = Array.isArray(draft.document.active?.dustAreas) ? draft.document.active.dustAreas : null;
      if (!areas?.length) return;
      const selectedAreaId = typeof options.areaId === "string" && options.areaId.trim()
        ? options.areaId.trim()
        : typeof draft.interaction.selectedDustAreaId === "string" && draft.interaction.selectedDustAreaId.trim()
          ? draft.interaction.selectedDustAreaId.trim()
          : null;
      const selectedAreaIndex = Number.isInteger(draft.interaction.selectedDustAreaIndex) ? draft.interaction.selectedDustAreaIndex : -1;
      const resolvedIndex = selectedAreaId ? areas.findIndex((area) => area?.id === selectedAreaId) : selectedAreaIndex;
      if (!Number.isInteger(resolvedIndex) || resolvedIndex < 0 || resolvedIndex >= areas.length) return;
      const previousArea = { ...areas[resolvedIndex] };
      const nextArea = updateDustAreaField(previousArea, field, value);
      if (JSON.stringify(previousArea) === JSON.stringify(nextArea)) return;
      areas[resolvedIndex] = nextArea;
      pushHistoryEntry(draft.history, createDustAreaEditEntry("update", {
        objectId: typeof nextArea.id === "string" ? nextArea.id : null,
        index: resolvedIndex,
        previousSnapshot: previousArea,
        nextSnapshot: nextArea,
  ...
  const deleteSelectedSmokeArea = (draft) => {
    const areas = Array.isArray(draft.document.active?.smokeAreas) ? draft.document.active.smokeAreas : null;
    if (!areas?.length) return false;
    const selectedId = typeof draft.interaction.selectedSmokeAreaId === "string" && draft.interaction.selectedSmokeAreaId.trim() ? draft.interaction.selectedSmokeAreaId.trim() : null;
    const selectedIndex = selectedId ? areas.findIndex((area) => area?.id === selectedId) : draft.interaction.selectedSmokeAreaIndex;
    if (!Number.isInteger(selectedIndex) || selectedIndex < 0 || selectedIndex >= areas.length) return false;
    const [removed] = areas.splice(selectedIndex, 1);
    pushHistoryEntry(draft.history, createSmokeAreaEditEntry("delete", {
      objectId: typeof removed?.id === "string" ? removed.id : null,
      index: selectedIndex,
      previousSnapshot: removed,
      nextSnapshot: null,
    }));
    setSmokeAreaSelection(draft, null);
    return true;
  };

  const updateSmokeArea = (field, value, options = {}) => {
    const supportedFields = new Set(["x", "y", "width", "height", "density", "size", "strength", "direction", "speed", "enabled", "visible"]);
    if (!supportedFields.has(field)) return;
    store.setState((draft) => {
      const areas = Array.isArray(draft.document.active?.smokeAreas) ? draft.document.active.smokeAreas : null;
      if (!areas?.length) return;
      const selectedAreaId = typeof options.areaId === "string" && options.areaId.trim()
        ? options.areaId.trim()
        : typeof draft.interaction.selectedSmokeAreaId === "string" && draft.interaction.selectedSmokeAreaId.trim()
          ? draft.interaction.selectedSmokeAreaId.trim()
          : null;
      const selectedAreaIndex = Number.isInteger(draft.interaction.selectedSmokeAreaIndex) ? draft.interaction.selectedSmokeAreaIndex : -1;
      const resolvedIndex = selectedAreaId ? areas.findIndex((area) => area?.id === selectedAreaId) : selectedAreaIndex;
      if (!Number.isInteger(resolvedIndex) || resolvedIndex < 0 || resolvedIndex >= areas.length) return;
      const previousArea = { ...areas[resolvedIndex] };
      const nextArea = updateSmokeAreaField(previousArea, field, value);
      if (JSON.stringify(previousArea) === JSON.stringify(nextArea)) return;
      areas[resolvedIndex] = nextArea;
      pushHistoryEntry(draft.history, createSmokeAreaEditEntry("update", {
        objectId: typeof nextArea.id === "string" ? nextArea.id : null,
        index: resolvedIndex,
        previousSnapshot: previousArea,
        nextSnapshot: nextArea,
      }));
      setSmokeAreaSelection(draft, resolvedIndex);
    });
  };

  const deleteSelectedWaterDropArea = (draft) => {
    const areas = Array.isArray(draft.document.active?.waterDropAreas) ? draft.document.active.waterDropAreas : null;
    if (!areas?.length) return false;
    const selectedId = typeof draft.interaction.selectedWaterDropAreaId === "string" && draft.interaction.selectedWaterDropAreaId.trim() ? draft.interaction.selectedWaterDropAreaId.trim() : null;
    const selectedIndex = selectedId ? areas.findIndex((area) => area?.id === selectedId) : draft.interaction.selectedWaterDropAreaIndex;
    if (!Number.isInteger(selectedIndex) || selectedIndex < 0 || selectedIndex >= areas.length) return false;
    const [removed] = areas.splice(selectedIndex, 1);
    pushHistoryEntry(draft.history, createWaterDropAreaEditEntry("delete", { objectId: typeof removed?.id === "string" ? removed.id : null, index: selectedIndex, previousSnapshot: removed, nextSnapshot: null }));
    setWaterDropAreaSelection(draft, null);
    return true;
  };

  const updateWaterDropArea = (field, value, options = {}) => {
    const supportedFields = new Set(["x", "y", "mode", "density", "speed", "size", "length", "enabled", "visible"]);
    if (!supportedFields.has(field)) return;
    store.setState((draft) => {
      const areas = Array.isArray(draft.document.active?.waterDropAreas) ? draft.document.active.waterDropAreas : null;
      if (!areas?.length) return;
      const selectedAreaId = typeof options.areaId === "string" && options.areaId.trim() ? options.areaId.trim() : typeof draft.interaction.selectedWaterDropAreaId === "string" && draft.interaction.selectedWaterDropAreaId.trim() ? draft.interaction.selectedWaterDropAreaId.trim() : null;
      const selectedAreaIndex = Number.isInteger(draft.interaction.selectedWaterDropAreaIndex) ? draft.interaction.selectedWaterDropAreaIndex : -1;
      const resolvedIndex = selectedAreaId ? areas.findIndex((area) => area?.id === selectedAreaId) : selectedAreaIndex;
      if (!Number.isInteger(resolvedIndex) || resolvedIndex < 0 || resolvedIndex >= areas.length) return;
      const previousArea = { ...areas[resolvedIndex] };
      const nextArea = updateWaterDropAreaField(previousArea, field, value);
      if (JSON.stringify(previousArea) === JSON.stringify(nextArea)) return;
      areas[resolvedIndex] = nextArea;
      pushHistoryEntry(draft.history, createWaterDropAreaEditEntry("update", { objectId: typeof nextArea.id === "string" ? nextArea.id : null, index: resolvedIndex, previousSnapshot: previousArea, nextSnapshot: nextArea }));
      setWaterDropAreaSelection(draft, resolvedIndex);
    });
  };
  ...
  const handleInspectCanvasMouseDown = (event, state, cell, point) => {
    ...
    const hitSmokeAreaIndex = findSmokeAreaAtCanvasPoint(state.document.active, state.viewport, point.x, point.y);
    const hitWaterDropAreaIndex = findWaterDropAreaAtCanvasPoint(state.document.active, state.viewport, point.x, point.y);
    const activeEntityPresetId = state.interaction.activeEntityPresetId;
    ...
    if (activeLayer === PANEL_LAYERS.WORLD_AREAS && state.interaction.activeWorldAreaType === "mirror_surface" && isMomentaryPlacementTrigger(event)) {
      interactionState.suppressNextClick = true;
      event.preventDefault();
      store.setState((draft) => {
        draft.interaction.hoverCell = cell;
        draft.interaction.selectedCell = cell;
        draft.interaction.mirrorSurfaceAreaPlacementDrag = {
          active: true,
          type: "mirror_surface",
          startCell: { ...cell },
          endCell: { ...cell },
    ...
    if (activeLayer === PANEL_LAYERS.WORLD_AREAS && state.interaction.activeWorldAreaType === "stone_area" && isMomentaryPlacementTrigger(event)) {
      interactionState.suppressNextClick = true;
      event.preventDefault();
      store.setState((draft) => {
        draft.interaction.hoverCell = cell;
        draft.interaction.selectedCell = cell;
        draft.interaction.stoneAreaPlacementDrag = {
          active: true,
          type: "stone_area",
          startCell: { ...cell },
          endCell: { ...cell },
    ...
    if (activeLayer === PANEL_LAYERS.WORLD_AREAS && state.interaction.activeWorldAreaType === "smoke_area" && isMomentaryPlacementTrigger(event)) {
      interactionState.suppressNextClick = true;
      event.preventDefault();
      store.setState((draft) => {
        draft.interaction.hoverCell = cell;
        draft.interaction.selectedCell = cell;
        draft.interaction.smokeAreaPlacementDrag = {
          active: true,
          type: "smoke_area",
          startCell: { ...cell },
          endCell: { ...cell },
    ...
    if (activeLayer === PANEL_LAYERS.WORLD_AREAS && state.interaction.activeWorldAreaType === "water_drop_area" && isMomentaryPlacementTrigger(event)) {
      interactionState.suppressNextClick = true;
      event.preventDefault();
      store.setState((draft) => {
        draft.interaction.hoverCell = cell;
        draft.interaction.selectedCell = cell;
        draft.interaction.waterDropAreaPlacementDrag = { active: true, type: "water_drop_area", startCell: { ...cell }, endCell: { ...cell } };
        clearReactiveGrassPatchSelection(draft.interaction);
        clearMirrorSurfaceAreaSelection(draft.interaction);
        clearStoneAreaSelection(draft.interaction);
        clearDustAreaSelection(draft.interaction);
    ...
    if (activeLayer === PANEL_LAYERS.WORLD_AREAS && state.interaction.activeWorldAreaType === "dust_area" && isMomentaryPlacementTrigger(event)) {
      interactionState.suppressNextClick = true;
      event.preventDefault();
      store.setState((draft) => {
        draft.interaction.hoverCell = cell;
        draft.interaction.selectedCell = cell;
        draft.interaction.dustAreaPlacementDrag = {
          active: true,
          type: "dust_area",
          startCell: { ...cell },
          endCell: { ...cell },
...
if (event.shiftKey) {
    ...
    if (activeLayer === PANEL_LAYERS.WORLD_AREAS) {
      ...
      store.setState((draft) => {
        ...
        if (hitWaterDropAreaIndex >= 0) {
          setWaterDropAreaSelection(draft, hitWaterDropAreaIndex);
          const area = draft.document.active?.waterDropAreas?.[hitWaterDropAreaIndex];
          draft.interaction.waterDropAreaDrag = { active: true, areaId: area?.id || null, anchorCell: { ...cell }, originArea: area ? { ...area } : null, previewDelta: { x: 0, y: 0 } };
          return;
        }
        if (hitSmokeAreaIndex >= 0) {
          setSmokeAreaSelection(draft, hitSmokeAreaIndex);
          const area = draft.document.active?.smokeAreas?.[hitSmokeAreaIndex];
          draft.interaction.smokeAreaDrag = {
            active: true,
            areaId: area?.id || null,
            anchorCell: { ...cell },
            originArea: area ? { ...area } : null,
        ...
        if (hitDustAreaIndex >= 0) {
          ...
          draft.interaction.dustAreaDrag = {
            active: true,
            areaId: area?.id || null,
            anchorCell: { ...cell },
            originArea: area ? { ...area } : null,
        ...
        if (hitStoneAreaIndex >= 0) {
          setStoneAreaSelection(draft, hitStoneAreaIndex);
          const area = draft.document.active?.stoneAreas?.[hitStoneAreaIndex];
          draft.interaction.stoneAreaDrag = {
            active: true,
            areaId: area?.id || null,
            anchorCell: { ...cell },
            originArea: area ? { ...area } : null,
        ...
        setMirrorSurfaceAreaSelection(draft, hitMirrorSurfaceAreaIndex);
        if (hitMirrorSurfaceAreaIndex >= 0 && draft.interaction.selectedMirrorSurfaceAreaIndex === hitMirrorSurfaceAreaIndex) {
          const area = draft.document.active?.mirrorSurfaceAreas?.[hitMirrorSurfaceAreaIndex];
          draft.interaction.mirrorSurfaceAreaDrag = {
            active: true,
            areaId: area?.id || null,
            anchorCell: { ...cell },
            originArea: area ? { ...area } : null,
  ...
  const handleCanvasMouseMove = (event) => {
    ...
    if (state.interaction.mirrorSurfaceAreaDrag?.active || state.interaction.stoneAreaDrag?.active || state.interaction.dustAreaDrag?.active || state.interaction.glowAreaDrag?.active || state.interaction.smokeAreaDrag?.active || state.interaction.waterDropAreaDrag?.active) {
      if ((event.buttons & 1) !== 1) return;
      const point = getCanvasPointFromMouseEvent(canvas, event);
      const cell = getCellFromCanvasPoint(state.document.active, state.viewport, point.x, point.y);
      if (!cell) return;
      store.setState((draft) => {
        const drag = draft.interaction.mirrorSurfaceAreaDrag || draft.interaction.stoneAreaDrag || draft.interaction.dustAreaDrag || draft.interaction.glowAreaDrag || draft.interaction.smokeAreaDrag || draft.interaction.waterDropAreaDrag;
    ...
    if (state.interaction.mirrorSurfaceAreaPlacementDrag?.active || state.interaction.stoneAreaPlacementDrag?.active || state.interaction.dustAreaPlacementDrag?.active || state.interaction.glowAreaPlacementDrag?.active || state.interaction.smokeAreaPlacementDrag?.active || state.interaction.waterDropAreaPlacementDrag?.active) {
      if ((event.buttons & 1) !== 1) return;
      const point = getCanvasPointFromMouseEvent(canvas, event);
      const cell = getCellFromCanvasPoint(state.document.active, state.viewport, point.x, point.y);
      if (!cell) return;
      store.setState((draft) => {
        const placementDrag = draft.interaction.mirrorSurfaceAreaPlacementDrag || draft.interaction.stoneAreaPlacementDrag || draft.interaction.dustAreaPlacementDrag || draft.interaction.glowAreaPlacementDrag || draft.interaction.smokeAreaPlacementDrag || draft.interaction.waterDropAreaPlacementDrag;
  ...
  const stopCanvasInteraction = () => {
    ...
    if (state.interaction.mirrorSurfaceAreaDrag?.active) {
      store.setState((draft) => {
        const drag = draft.interaction.mirrorSurfaceAreaDrag;
        if (!drag?.active || !drag.originArea) return;
        const areas = Array.isArray(draft.document.active?.mirrorSurfaceAreas) ? draft.document.active.mirrorSurfaceAreas : [];
        const index = drag.areaId ? areas.findIndex((area) => area?.id === drag.areaId) : draft.interaction.selectedMirrorSurfaceAreaIndex;
        ...
        const nextArea = moveMirrorSurfaceArea(drag.originArea, delta.x, delta.y);
        if (JSON.stringify(previousArea) !== JSON.stringify(nextArea)) {
          ...
          setMirrorSurfaceAreaSelection(draft, index);
        }
        draft.interaction.mirrorSurfaceAreaDrag = null;
      });
      return;
    ...
    if (state.interaction.stoneAreaDrag?.active) {
      store.setState((draft) => {
        const drag = draft.interaction.stoneAreaDrag;
        if (!drag?.active || !drag.originArea) return;
        const areas = Array.isArray(draft.document.active?.stoneAreas) ? draft.document.active.stoneAreas : [];
        const index = drag.areaId ? areas.findIndex((area) => area?.id === drag.areaId) : draft.interaction.selectedStoneAreaIndex;
        if (!Number.isInteger(index) || index < 0 || index >= areas.length) {
          draft.interaction.stoneAreaDrag = null;
          return;
        }
        const previousArea = { ...areas[index] };
        const delta = drag.previewDelta || { x: 0, y: 0 };
        const nextArea = moveStoneArea(drag.originArea, delta.x, delta.y);
        if (JSON.stringify(previousArea) !== JSON.stringify(nextArea)) {
          areas[index] = nextArea;
          pushHistoryEntry(draft.history, createStoneAreaEditEntry("update", {
            objectId: typeof nextArea.id === "string" ? nextArea.id : null,
          ...
          setStoneAreaSelection(draft, index);
        }
        draft.interaction.stoneAreaDrag = null;
      });
      return;
    ...
    if (state.interaction.dustAreaDrag?.active) {
      store.setState((draft) => {
        const drag = draft.interaction.dustAreaDrag;
        if (!drag?.active || !drag.originArea) return;
        const areas = Array.isArray(draft.document.active?.dustAreas) ? draft.document.active.dustAreas : [];
        const index = drag.areaId ? areas.findIndex((area) => area?.id === drag.areaId) : draft.interaction.selectedDustAreaIndex;
        if (!Number.isInteger(index) || index < 0 || index >= areas.length) {
          draft.interaction.dustAreaDrag = null;
          return;
        }
        const previousArea = { ...areas[index] };
        const delta = drag.previewDelta || { x: 0, y: 0 };
        const nextArea = moveDustArea(drag.originArea, delta.x, delta.y);
        if (JSON.stringify(previousArea) !== JSON.stringify(nextArea)) {
          areas[index] = nextArea;
          pushHistoryEntry(draft.history, createDustAreaEditEntry("update", {
            objectId: typeof nextArea.id === "string" ? nextArea.id : null,
          ...
          setDustAreaSelection(draft, index);
        }
        draft.interaction.dustAreaDrag = null;
      });
      return;
    ...
    if (state.interaction.waterDropAreaDrag?.active) {
      store.setState((draft) => {
        const drag = draft.interaction.waterDropAreaDrag;
        if (!drag?.active || !drag.originArea) return;
        const areas = Array.isArray(draft.document.active?.waterDropAreas) ? draft.document.active.waterDropAreas : [];
        const index = drag.areaId ? areas.findIndex((area) => area?.id === drag.areaId) : draft.interaction.selectedWaterDropAreaIndex;
        if (!Number.isInteger(index) || index < 0 || index >= areas.length) { draft.interaction.waterDropAreaDrag = null; return; }
        const previousArea = { ...areas[index] };
        const delta = drag.previewDelta || { x: 0, y: 0 };
        const nextArea = moveWaterDropArea(drag.originArea, delta.x, delta.y);
        if (JSON.stringify(previousArea) !== JSON.stringify(nextArea)) {
          areas[index] = nextArea;
          pushHistoryEntry(draft.history, createWaterDropAreaEditEntry("update", { objectId: typeof nextArea.id === "string" ? nextArea.id : null, index, previousSnapshot: previousArea, nextSnapshot: nextArea }));
          setWaterDropAreaSelection(draft, index);
        }
        draft.interaction.waterDropAreaDrag = null;
      });
      return;
    }

    if (state.interaction.smokeAreaDrag?.active) {
      store.setState((draft) => {
        const drag = draft.interaction.smokeAreaDrag;
        if (!drag?.active || !drag.originArea) return;
        const areas = Array.isArray(draft.document.active?.smokeAreas) ? draft.document.active.smokeAreas : [];
        const index = drag.areaId ? areas.findIndex((area) => area?.id === drag.areaId) : draft.interaction.selectedSmokeAreaIndex;
        if (!Number.isInteger(index) || index < 0 || index >= areas.length) {
          draft.interaction.smokeAreaDrag = null;
          return;
        }
        const previousArea = { ...areas[index] };
        const delta = drag.previewDelta || { x: 0, y: 0 };
        const nextArea = moveSmokeArea(drag.originArea, delta.x, delta.y);
        if (JSON.stringify(previousArea) !== JSON.stringify(nextArea)) {
          areas[index] = nextArea;
          pushHistoryEntry(draft.history, createSmokeAreaEditEntry("update", {
            objectId: typeof nextArea.id === "string" ? nextArea.id : null,
          ...
          setSmokeAreaSelection(draft, index);
        }
        draft.interaction.smokeAreaDrag = null;
      });
      return;
    ...
    if (state.interaction.mirrorSurfaceAreaPlacementDrag?.active) {
      store.setState((draft) => {
        ...
        if (!placementDrag?.active) return;
        const createdArea = createMirrorSurfaceAreaFromDrag(
          draft.document.active,
          placementDrag.startCell,
          placementDrag.endCell || placementDrag.startCell,
        );
        if (createdArea) {
          if (!Array.isArray(draft.document.active.mirrorSurfaceAreas)) draft.document.active.mirrorSurfaceAreas = [];
          const createdIndex = draft.document.active.mirrorSurfaceAreas.length;
          draft.document.active.mirrorSurfaceAreas.push(createdArea);
          pushHistoryEntry(draft.history, createMirrorSurfaceAreaEditEntry("create", {
            objectId: typeof createdArea.id === "string" ? createdArea.id : null,
            index: createdIndex,
            nextSnapshot: createdArea,
          }));
          setMirrorSurfaceAreaSelection(draft, createdIndex);
        }
        draft.interaction.mirrorSurfaceAreaPlacementDrag = null;
      });
      return;
    ...
    if (state.interaction.stoneAreaPlacementDrag?.active) {
      store.setState((draft) => {
        const placementDrag = draft.interaction.stoneAreaPlacementDrag;
        if (!placementDrag?.active) return;
        const createdArea = createStoneAreaFromDrag(
          draft.document.active,
          placementDrag.startCell,
          placementDrag.endCell || placementDrag.startCell,
        );
        if (createdArea) {
          if (!Array.isArray(draft.document.active.stoneAreas)) draft.document.active.stoneAreas = [];
          const createdIndex = draft.document.active.stoneAreas.length;
          draft.document.active.stoneAreas.push(createdArea);
          pushHistoryEntry(draft.history, createStoneAreaEditEntry("create", {
            objectId: typeof createdArea.id === "string" ? createdArea.id : null,
            index: createdIndex,
            nextSnapshot: createdArea,
          }));
          setStoneAreaSelection(draft, createdIndex);
        }
        draft.interaction.stoneAreaPlacementDrag = null;
      });
      return;
    ...
    if (state.interaction.dustAreaPlacementDrag?.active) {
      store.setState((draft) => {
        const placementDrag = draft.interaction.dustAreaPlacementDrag;
        if (!placementDrag?.active) return;
        const createdArea = createDustAreaFromDrag(
          draft.document.active,
          placementDrag.startCell,
          placementDrag.endCell || placementDrag.startCell,
        );
        if (createdArea) {
          if (!Array.isArray(draft.document.active.dustAreas)) draft.document.active.dustAreas = [];
          const createdIndex = draft.document.active.dustAreas.length;
          draft.document.active.dustAreas.push(createdArea);
          pushHistoryEntry(draft.history, createDustAreaEditEntry("create", {
            objectId: typeof createdArea.id === "string" ? createdArea.id : null,
            index: createdIndex,
            nextSnapshot: createdArea,
          }));
    ...
    if (state.interaction.waterDropAreaPlacementDrag?.active) {
      store.setState((draft) => {
        const placementDrag = draft.interaction.waterDropAreaPlacementDrag;
        if (!placementDrag?.active) return;
        const createdArea = createWaterDropAreaFromDrag(draft.document.active, placementDrag.startCell, placementDrag.endCell || placementDrag.startCell);
        if (createdArea) {
          if (!Array.isArray(draft.document.active.waterDropAreas)) draft.document.active.waterDropAreas = [];
          const createdIndex = draft.document.active.waterDropAreas.length;
          draft.document.active.waterDropAreas.push(createdArea);
          pushHistoryEntry(draft.history, createWaterDropAreaEditEntry("create", { objectId: typeof createdArea.id === "string" ? createdArea.id : null, index: createdIndex, previousSnapshot: null, nextSnapshot: createdArea }));
          setWaterDropAreaSelection(draft, createdIndex);
        }
        draft.interaction.waterDropAreaPlacementDrag = null;
      });
      return;
    ...
    if (state.interaction.smokeAreaPlacementDrag?.active) {
      store.setState((draft) => {
        const placementDrag = draft.interaction.smokeAreaPlacementDrag;
        if (!placementDrag?.active) return;
        const createdArea = createSmokeAreaFromDrag(
          draft.document.active,
          placementDrag.startCell,
          placementDrag.endCell || placementDrag.startCell,
        );
        if (createdArea) {
          if (!Array.isArray(draft.document.active.smokeAreas)) draft.document.active.smokeAreas = [];
          const createdIndex = draft.document.active.smokeAreas.length;
          draft.document.active.smokeAreas.push(createdArea);
          pushHistoryEntry(draft.history, createSmokeAreaEditEntry("create", {
            objectId: typeof createdArea.id === "string" ? createdArea.id : null,
            index: createdIndex,
            previousSnapshot: null,
            nextSnapshot: createdArea,
          }));
          setSmokeAreaSelection(draft, createdIndex);
        }
        draft.interaction.smokeAreaPlacementDrag = null;
      });
      return;
  ...
  const setActiveLayerFromPanel = (layer) => {
    store.setState((draft) => {
      ...
      if (layer === PANEL_LAYERS.WORLD_AREAS) {
        applyCanvasTarget(draft, "worldAreas");
        return;
      }
  ...
  const handleGlobalKeyDown = (event) => {
    ...
    if ((event.key === "Delete" || event.key === "Backspace")) {
      ...
      store.setState((draft) => {
        ...
        if (activeLayer === PANEL_LAYERS.WORLD_AREAS) {
          if (!deleteSelectedWaterDropArea(draft) && !deleteSelectedSmokeArea(draft) && !deleteSelectedGlowArea(draft) && !deleteSelectedDustArea(draft) && !deleteSelectedStoneArea(draft)) deleteSelectedMirrorSurfaceArea(draft);
        }
      });
      return;
    }
  ...
  const syncWizardDecorPresetIdHint = (wizard) => {
    if (!wizard || wizard.assetType !== ASSET_WIZARD_TYPES.DECOR || wizard.mode !== ASSET_WIZARD_MODES.CREATE) return;
    wizard.draft = wizard.draft || {};
    const suggestedPresetId = suggestDecorPresetId({
      displayName: wizard.draft.displayName,
      spriteFileName: wizard.draft.spriteFileName,
  ...
  const attemptDecorWizardSave = async () => {
    const currentWizard = store.getState().ui.assetManager.wizard || createInitialAssetManagerWizardState();
    const identityValidation = getAssetWizardStepValidation("identity", currentWizard);
    const metadataValidation = getAssetWizardStepValidation("metadata", currentWizard);
    if (!identityValidation.isValid || !metadataValidation.isValid) {
    ...
    const drawH = Number.parseInt(resolvedDraft.drawHeight, 10);
    const footprint = parseFootprint(resolvedDraft.footprint);
    const bridgeResult = await saveDecorThroughLocalBridge({
      draft: resolvedDraft,
      decorPayload: {
    ...
    const registerResult = registerDecorPresetOption({
      presetId: bridgeResult.persistedDecor?.presetId || resolvedDraft.presetId,
      type: bridgeResult.persistedDecor?.type || bridgeResult.persistedDecor?.presetId || resolvedDraft.presetId,
      defaultName: bridgeResult.persistedDecor?.label || resolvedDraft.displayName,
      img: bridgeResult.persistedDecor?.img || null,
      drawW: bridgeResult.persistedDecor?.drawW || drawW,
      drawH: bridgeResult.persistedDecor?.drawH || drawH,
      drawAnchor: bridgeResult.persistedDecor?.drawAnchor || resolvedDraft.drawAnchor,
      drawOffX: bridgeResult.persistedDecor?.drawOffX || 0,
      drawOffY: bridgeResult.persistedDecor?.drawOffY || 0,
      footprint: bridgeResult.persistedDecor?.footprint || footprint,
    ...
    if (!registerResult.ok) {
      store.setState((draft) => {
        ...
        wizard.draft.saveFeedback = "Decor saved on disk, but editor catalog refresh failed. Reload the page.";
        draft.ui.assetManager.wizard = wizard;
      });
      return false;
    }
    ...
    store.setState((draft) => {
      const wizard = draft.ui.assetManager.wizard || createInitialAssetManagerWizardState();
      setActiveLayer(draft, PANEL_LAYERS.DECOR);
      draft.interaction.activeTool = EDITOR_TOOLS.PAINT;
      draft.interaction.activeDecorPresetId = registerResult.preset.id;
      draft.ui.panelSections.decor = true;
      draft.ui.assetManager.selectedCategory = "decor";
      draft.ui.assetManager.activeView = "wizard";
      draft.ui.importStatus = `Decor created (persisted): ${registerResult.preset.defaultName || registerResult.preset.id}`;
      cleanupWizardPreview(wizard);
      draft.ui.assetManager.wizard = createInitialAssetManagerWizardState();
      draft.ui.assetManager.isOpen = false;
      selectedAssetWizardSpriteFile = null;
  ...
  const loadDocument = async () => {
    ...
    try {
      ...
      store.setState((state) => {
        ...
        state.interaction.mirrorSurfaceAreaPlacementDrag = null;
        state.interaction.mirrorSurfaceAreaDrag = null;
        ...
        state.interaction.activeWorldAreaType = null;
  ...
  const panelBindingOptions = {
    ...
    onSmokeAreaUpdate: updateSmokeArea,
    onWaterDropAreaUpdate: updateWaterDropArea,
  ...
  const unbindBrushPanel = bindBrushPanel(brushPanel, store, {
    ...
    onWorldAreaUpdate: updateWorldArea,

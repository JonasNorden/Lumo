...
for (const stone of stones) {
  ...
  assert.ok(bottomY <= baselineY + 0.01, `stone ${stone.id} bottom must not pass below baseline`);
  assert.ok(baselineY - bottomY <= 1.22, `stone ${stone.id} should rest near baseline instead of floating (${baselineY - bottomY}px gap)`);
}
...
assert.equal(firstVisual.kind, "stylized-faceted-polygon", "stone visuals use the faceted polygon geometry contract");
assert.ok(Array.isArray(firstVisual.facets) && firstVisual.facets.length >= 4, "stone visual exposes multiple facets");
...
assert.match(runtimeSource, /kind: "stylized-faceted-polygon"/, "runtime uses the same faceted polygon geometry kind as editor");
